...
describe('POST /admin/api/asaas', () => {
  ...
  it('retorna 403 quando autenticacao falha', async () => {
    (requireClienteFromHost as unknown as { mockReturnValue: (v: any) => void }).mockResolvedValue({
      error: 'Acesso negado',
      status: 403
    })
  ...
  it('envia billingType correto no payload', async () => {
    ...
    ;(requireClienteFromHost as unknown as { mockReturnValue: (v: any) => void }).mockResolvedValue({ pb, cliente: { nome: 'Cli', asaas_api_key: 'key' } })
