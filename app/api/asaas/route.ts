...
export async function POST(req: NextRequest) {
  ...
  if ('error' in auth) {
    await logConciliacaoErro(`Erro de autenticação: ${auth.error}`)
  ...
  if (!apiKey) {
    await logConciliacaoErro('ASAAS_API_KEY não definida!')
    throw new Error(
  ...
  if (!keyHeader || !baseUrl) {
    await logConciliacaoErro('Chave da API Asaas ou URL não configurada')
